...
type Manifest struct {
	...
	sha256                  string
	order                   []candidate
	auditIncludeGlobs       []*Glob
	auditIncludeErrors      []patternError
	auditExcludeGlobs       []*Glob
	auditExcludeErrors      []patternError
	generatedArtifactGlobs  []*Glob
	generatedArtifactErrors []patternError
}
...
func (m *Manifest) compile() {
	m.auditIncludeGlobs, m.auditIncludeErrors = compilePatterns(m.AuditScan.Include)
	m.auditExcludeGlobs, m.auditExcludeErrors = compilePatterns(m.AuditScan.Exclude)
	m.generatedArtifactGlobs, m.generatedArtifactErrors = compilePatterns(m.GeneratedArtifacts)
...
func compilePatterns(patterns []string) ([]*Glob, []patternError) {
...
// InAuditScan reports whether a path is in the high-availability vocabulary
// scan. A broken include widens the scan to every path so it cannot silence V9;
// broken excludes match nothing and therefore cannot narrow the scan.
func (m *Manifest) InAuditScan(path string) bool {
	included := len(m.AuditScan.Include) == 0 || len(m.auditIncludeErrors) > 0
	if !included {
		for _, g := range m.auditIncludeGlobs {
			if g.Match(path) {
				included = true
	...
	for _, g := range m.auditExcludeGlobs {
		if g.Match(path) {
			return false
		}
	}
...
func (m *Manifest) IsGeneratedArtifact(path string) bool {
	// Invalid patterns match nothing. That unsafe direction is unreachable in
	// reports because runReport refuses structural errors before building one;
	// treating every path as generated would demand regeneration for every change.
	for _, g := range m.generatedArtifactGlobs {
		if g.Match(path) {
			return true
		}
	}
