...
# 6-instance CNPG ANY 2 adversarial logged-data failover-quorum skew repro.
#
# Goal: test whether a stale/skewed FailoverQuorum.Status can make
# failoverQuorum=true unsafe. Topology:
#   side A = old primary + two actual synchronous standbys
#   side B = operator/API-visible side + three standbys listed in FQ status
#
# We keep side-A PostgreSQL replication paths alive so the old primary can still
# satisfy ANY 2 synchronous_commit=on. We then patch FailoverQuorum.Status toward
# side B, stop kubelet on side A, and force-delete old-side Pod objects so
# Kubernetes/CNPG can promote while old PostgreSQL processes are not fenced.
#
# This is an injected metadata-skew experiment, not proof that CNPG naturally
# produces this skew.

CLUSTER_NAME=${CLUSTER_NAME:-cnpg-sync6-logged-fq-skew}
...
SKEW_FAILOVER_QUORUM=${SKEW_FAILOVER_QUORUM:-true}
HIDE_OLD_SIDE_STANDBYS=${HIDE_OLD_SIDE_STANDBYS:-true}
...
sample(){
  ...
  out="$EVIDENCE_DIR/$(ts)-sample-$label.txt"
  {
    ...
    echo "## failoverquorum"
    kubectl get failoverquorum -n "$NAMESPACE" "$PG_CLUSTER" -o yaml || true
    p=$(get_primary)
    if [[ -n "$p" ]]; then
...
if [[ "$SKEW_FAILOVER_QUORUM" == "true" ]]; then
  PROMOTION_STANDBYS=()
  for s in "${SYNC_STANDBYS[@]}"; do
    skip=0
    for k in "${KEEP_STANDBYS[@]}"; do [[ "$s" == "$k" ]] && skip=1; done
    [[ "$skip" == 0 ]] && PROMOTION_STANDBYS+=("$s")
  done
  if [[ ${#PROMOTION_STANDBYS[@]} -lt 3 ]]; then echo "Need >=3 promotion-side standbys" >&2; exit 2; fi
  PATCH_JSON=$(PROMOTION_NAMES="${PROMOTION_STANDBYS[*]}" PRIMARY_NAME="$PRIMARY_POD" python3 - <<'PY2'
import json, os
names=os.environ["PROMOTION_NAMES"].split()
print(json.dumps({"status":{"method":"any","primary":os.environ["PRIMARY_NAME"],"standbyNumber":2,"standbyNames":names}}))
PY2
)
  echo "$PATCH_JSON" >"$EVIDENCE_DIR/failoverquorum-skew-patch.json"
  echo "$(date +%Y-%m-%dT%H:%M:%S%z) skew-failoverquorum promotion_side=${PROMOTION_STANDBYS[*]}" | tee -a "$EVIDENCE_DIR/timeline.log"
  run kubectl patch failoverquorum -n "$NAMESPACE" "$PG_CLUSTER" --subresource=status --type=merge -p "$PATCH_JSON"
  kubectl get failoverquorum -n "$NAMESPACE" "$PG_CLUSTER" -o yaml >"$EVIDENCE_DIR/failoverquorum-after-skew.yaml" || true
fi

if [[ "$HIDE_OLD_SIDE_STANDBYS" == "true" ]]; then
  log "Stop kubelet on kept-standby nodes and force-delete their Pod objects while preserving containers"
  for idx in "${!KEEP_STANDBYS[@]}"; do
    pod="${KEEP_STANDBYS[$idx]}"; node="${KEEP_NODES[$idx]}"
    echo "$(date +%Y-%m-%dT%H:%M:%S%z) stop-kubelet-service kept-standby=$pod node=$node" | tee -a "$EVIDENCE_DIR/timeline.log"
    run docker exec "$node" bash -lc 'systemctl stop kubelet || true; systemctl disable kubelet || true; pkill -9 kubelet || true; systemctl is-active kubelet || true; ps -ef | grep kubelet | grep -v grep || true'
    echo "$(date +%Y-%m-%dT%H:%M:%S%z) force-delete-kept-standby-pod=$pod" | tee -a "$EVIDENCE_DIR/timeline.log"
    run kubectl delete pod -n "$NAMESPACE" "$pod" --grace-period=0 --force
  done
fi
...
{
  echo "# CNPG 6-instance ANY 2 logged-data failover-quorum status-skew split-brain"
  echo
  echo "## Timeline"
